...
# def calculate_business_uptime_downtime_for_day(observations, business_hours):

#     total_uptime = 0
#     total_downtime = 0
#     last_observation_time = None
#     max_date = max(obs['timestamp_utc'] for obs in observations)
#     filtered_observations = [obs for obs in observations if obs['timestamp_utc'].startswith(max_date)]

#     max_date_obj = parse_timestamp(max_date)
#     weekday = max_date_obj.weekday()
#     menu_hours= business_hours[weekday]
#     sorted_observations = sorted(filtered_observations, key=lambda x: parse_timestamp(x["timestamp_utc"]))
    
#     print(menu_hours)
#     opening_time=datetime.strptime(menu_hours[1], '%H:%M:%S').time()
#     closing_time=datetime.strptime(menu_hours[2], '%H:%M:%S').time()
#     opening_time=datetime.combine(max_date_obj, opening_time)
#     closing_time=datetime.combine(max_date_obj, closing_time)
#     latest_status = None
#     filtered_observations = [obs for obs in sorted_observations if parse_timestamp(obs["timestamp_utc"]).time() >= opening_time.time() and parse_timestamp(obs["timestamp_utc"]).time() < closing_time.time()]
#     total_obs=len(filtered_observations)
    
#     if(total_obs==0):
#         return 0,0
#     print(filtered_observations)
#     # last_observation_time=datetime.strptime(filtered_observations[0][2], '%Y-%m-%d %H:%M:%S').time()
#     last_observation_time=parse_timestamp(filtered_observations[0][2])
#     latest_status=filtered_observations[0][1]
#     i=1
#     if(latest_status=="active"):
#         total_uptime+=(last_observation_time-opening_time).total_seconds()
#     else:
#         total_downtime+=(last_observation_time-opening_time).total_seconds()
#     while(i<total_obs):
#         if(filtered_observations[1][1]==latest_status):
#             i=i+1
#         else:
#             if(latest_status=="active"):
#                 total_uptime+=(last_observation_time-parse_timestamp(filtered_observations[i][2])).total_seconds()
#             else:
#                 total_downtime+=(last_observation_time-parse_timestamp(filtered_observations[i][2])).total_seconds()

#     if(latest_status=="active"):
#         total_uptime+=(closing_time-parse_timestamp(filtered_observations[total_obs-1][2])).total_seconds()
#     else:
#         total_downtime(closing_time-parse_timestamp(filtered_observations[total_obs-1][2])).total_seconds()
